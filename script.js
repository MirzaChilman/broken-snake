...
function renderLife(ctx) {
    while (ekstra.position.y != 0 || wallCollision(ekstra.position.x, ekstra.position.y)) {
        ekstra.position = initPosition();
    }
    if (ekstra.visible) {
        var img = document.getElementById("life");
        ctx.drawImage(img, ekstra.position.x * CELL_SIZE, ekstra.position.y * CELL_SIZE, CELL_SIZE, CELL_SIZE);
        ekstra.visibleCount++;
        if (ekstra.visibleCount == 10) {
            ekstra.visible = false;
        }
    } else {
        drawCell(ctx, ekstra.position.x, ekstra.position.y, "rgb(255,255,255,0)")
        ekstra.visibleCount--;
        if (ekstra.visibleCount == 0) {
            ekstra.visible = true;
        }
    }
}
...
function eatExtraLife() {
    if (snake.head.x == ekstra.position.x || snake.head.y == ekstra.position.y) {
        ekstra.position = initPosition();
        snakeProp.life++;
        snakeProp.score++;
